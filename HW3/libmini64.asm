...
sleep_quit:
        ...
        global signal_rt:function
signal_rt:
        mov     rax, 15
        syscall

        extern save_signal_mask
        global setjmp:function
setjmp:
        push    rdi            ; save jmp_buf
        call    save_signal_mask WRT ..plt
        pop     rdi            ; load jmp_buf
        mov     [rdi+0], rbx   ; save the parameter to jmp_buf
        mov     [rdi+8], rsp   
        mov     [rdi+16], rbp
        mov     [rdi+24], r12
        mov     [rdi+32], r13
        mov     [rdi+40], r14
        mov     [rdi+48], r15
        pop     rsi            ; get the return address , save to rsi
        push    rsi            ; push back to stack
        mov     [rdi+56], rsi
        mov     rax, 0      ; return 0 ,
        ret

        extern load_signal_mask
        global longjmp:function
longjmp:
        push    rdi
        push    rsi
        call    load_signal_mask WRT ..plt
        pop     rsi
        pop     rdi
        mov     rbx, [rdi+0]   ; give the old parameters
        mov     rsp, [rdi+8] 
        mov     rbp, [rdi+16]
        mov     r12, [rdi+24]
        mov     r13, [rdi+32]
        ...
        mov     r15, [rdi+48]
        mov     rax, rsi
        jmp     [rdi+56]       ; go back to the return address
