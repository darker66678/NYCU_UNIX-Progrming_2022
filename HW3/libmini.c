...
int sigaddset(sigset_t *set, int sig)
{
    ...
    set->sig[(sig - 1) / _NSIG_BPW] |= 1UL << (sig - 1) % _NSIG_BPW;
    return 0;
}
...
void save_signal_mask(jmp_buf env)
{
    sigset_t current_mask;
    int ret = sigprocmask(SIG_BLOCK, NULL, &current_mask);
    if (ret >= 0)
    {
        env->mask = current_mask;
    }
    else
    {
        char err[23] = {"save signal mask error"};
        perror(err);
    }
}

void load_signal_mask(jmp_buf env)
{
    int ret = sigprocmask(SIG_SETMASK, &(env->mask), NULL);
    if (ret < 0)
    {
        char err[23] = {"load signal mask error"};
        perror(err);
    }
}
